...
if lastInputFunction.lower() == 'yes':
    FlashLightQuestion = input(
        'Inside the drawer of one of your cabinets is a small flashlight. Do you TAKE it?: ')
...
# #     print('you dress')
# # elif lastInputFunction == 'proceed':
# #     print('you leave')
...
if lastInputFunction.lower() in ('continue', 'go'):
    ...
    if lastInputFunction.lower() == 'check':
        ...
        if FlashLight:
            lastInputFunction = input(
                'Would you like to use your FLASHLIGHT? ')
            if lastInputFunction.lower() in ('yes', 'flashlight'):
                ...
                print(
                    'You do not use your flashlight, and hence cannot see anything, so you turn back around.')
        else:
...
if search:
    lastInputFunction = input(
        'Inside the closet is an old baseball bat of yours. \nDo you TAKE the bat? ')
else:
...
lastInputFunction = input(
    'Would you like to INVESTIGATE further, JUMP out the window, or CONTINUE? ')
if lastInputFunction.lower() in ('yes', 'search', 'investigate'):
    ...
    lastInputFunction = input(
        'you can run to the POLICE station, or you can run to the NEIGHBORS house. ')
    if lastInputFunction.lower() == 'police':
        ...
        lastInputFunction = input(
            'you can TAKE his gun, or you can file a REPORT. ')
        ...
        if lastInputFunction.lower() == 'help':
            print(
                'the neighbor obliges, grabs their shotgun, and together you defeat the intruder')
...
lastInputFunction = input(
    "The intruder approaches threateningly. You have to make you move. do you take an AGGRESSIVE action, and charge your foe, or do you take a DEFENSIVE action, and prepare to weather the intruder's assault")
